              ...
              <Tbody>
                {transactions?.map((tx, key) => (
                  <Tr key={key} _hover={{ bgColor: hoverTrColor }} cursor="pointer">
                    <Td color={'white'}>{getEllipsisTxt(tx?.hash || '')}</Td>
                    <Td color={'white'}>{getEllipsisTxt(tx?.from || '')}</Td>
                    <Td color={'white'}>{getEllipsisTxt(tx?.to || '')}</Td>
                    <Td color={'white'}>{tx.gasUsed}</Td>
                    <Td color={'white'}>{new Date(tx.blockTimestamp).toLocaleDateString()}</Td>
